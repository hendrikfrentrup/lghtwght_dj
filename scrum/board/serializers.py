    ...
    def get_links(self, obj):
            ...
            if self.instance and self.instance.pk:
                if value != self.instance.sprint:
                    ...
                    if value and value.end < date.today():
                        msg = _('Cannot assign tasks tp past sprints.')
                        raise serializers.ValidationError(msg)
            else:
                if value and value.end < date.today():
                    msg = _('Cannot add tasks tp past sprints.')
                    raise serializers.ValidationError(msg)
            return value
        ...
        def validate(self, attrs):
            ...
            if completed and status != Task.STATUS_DONE:
                msg = _('Completed date cannot be set for the uncompleted tasks.')
                raise serializers.ValidationError(msg)
            return attrs
